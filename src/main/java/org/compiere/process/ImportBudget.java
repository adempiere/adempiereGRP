 ...
 */
public class ImportBudget extends ImportBudgetAbstract  implements ImportProcess{
    ...
    protected String doIt() throws Exception {
        ...
        if (documentNo == null || documentNo.length() <= 0 )
            throw new AdempiereException("@BatchDocumentNo@ @IsMandatory@");
        ...
        Arrays.stream(budgetLinesIds).forEach(importBudgetId -> {
            Trx.run(trxName -> {
                        ...
                        ModelValidationEngine.get().fireImportValidate(this, importBudget, null , ImportValidator.TIMING_BEFORE_VALIDATE);
        ...
        Arrays.stream(budgetLinesIds).forEach(importBudgetId -> {
                        ...
                        if (importBudget.getI_ErrorMsg() != null)
                            throw new AdempiereException("@I_Budget_ID@ @ProcessFailed@ " + importBudget.getI_ErrorMsg());
    ...
    private void fillIdValues(X_I_Budget importBudget,  MAcctSchemaElement[] acctSchemaElements , String trxName) {
       ...
       if (importBudget.getAD_OrgTrx_ID() <= 0 && importBudget.getOrgTrxValue() != null)
        ...
        if (!stringError.toString().isEmpty() && stringError.toString().length() > 0) {
            String errorMsg = importBudget.getI_ErrorMsg() != null ? importBudget.getI_ErrorMsg() + " " + Msg.parseTranslation(getCtx(), stringError.toString()) :  Msg.parseTranslation(getCtx(), stringError.toString());
            importBudget.setI_ErrorMsg(errorMsg);
            importBudget.saveEx();
        }
        importBudget.saveEx();
    }
    ...
    private void createJournal(MJournalBatch journalBatch, int conversionTypeId, int calendarId, int budgetId) {
        ...
        for (int periodNo = 0; periodNo < noPeriods; periodNo++) {
            ...
            journal.load(journalBatch.get_TrxName());
            if (journal.getTotalDr().signum() == 0 && journal.getTotalCr().signum() == 0)
    ...
    private void createJournalLine(MJournal journal, String documentNo, int noOfMonth) {
        ...
        Arrays.stream(getBudget(documentNo, false, false)).forEach(importBudgetId -> {
            ...
            if (amount != null && amount.compareTo(BigDecimal.ZERO) != 0) {
                ...
                ModelValidationEngine.get().fireImportValidate(this, importBudget, journalLine, ImportValidator.TIMING_BEFORE_IMPORT);
                ...
                importBudget.saveEx();
                ModelValidationEngine.get().fireImportValidate(this, importBudget, journalLine, ImportValidator.TIMING_AFTER_IMPORT);
            }
        });
    }
