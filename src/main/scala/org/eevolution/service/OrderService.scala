...
object OrderService {
  ...
  /**
    ...
    * @return
    */
  def afterNewOrderLine(orderLine: MOrderLine): String = {
    if (ValidCombinationService.existsBudgetValidCombination(orderLine)) {
      ...
      if (accountId > 0) ValidCombinationService.fillDimension(accountId, orderLine)(orderLine.getCtx, orderLine.get_TrxName())
    }
